...
class ErrorMonitor {
 public:
   ...
   */
  void update(const std::array<uint32_t, H2_NUM_MOTOR>& motorstates) {
 ...
 private:
  std::array<uint32_t, H2_NUM_MOTOR> prev_motorstate_;
  int error_count_ = 0;
  ...
  static const char* jointName(int index) {
    ...
    if (index >= 0 && index < H2_NUM_MOTOR) return names[index];
    return "Unknown";
  }
};
