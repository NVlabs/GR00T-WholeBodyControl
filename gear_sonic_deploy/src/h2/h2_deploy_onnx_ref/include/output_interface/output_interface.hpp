...
protected:
     ...
     *   body_q_target          | N    | Target joint positions (MuJoCo order, N = G1_NUM_MOTOR).
     *   base_trans_measured    |  3   | Measured base translation (fixed default).
     *   base_quat_measured     |  4   | Measured base quaternion from IMU.
     *   body_q_measured        | N    | Measured joint positions (MuJoCo order + default offsets).
    ...
    )
    {
        ...
        bool has_joint_data = motion_frame_valid && current_motion->GetNumJoints() >= H2_NUM_MOTOR;
        ...
        std::array<double, H2_NUM_MOTOR> body_q_target;
        ...
        std::array<double, H2_NUM_MOTOR> body_q_measured;
