...
#include "../policy_parameters.hpp"  // For isaaclab_to_mujoco, default_angles, H2_action_scale
...
private:
    ...
    std::vector<uint8_t> parse_state_logger_state_to_msgpack(const DataBuffer<HeadingState>& heading_state_buffer) {
        ...
        if (state.body_q.size() == static_cast<size_t>(H2_NUM_MOTOR)) {
            // Create temporary array for MuJoCo-ordered values
            std::array<double, H2_NUM_MOTOR> body_q_mujoco;
            for (size_t i = 0; i < static_cast<size_t>(H2_NUM_MOTOR); ++i) {
        ...
        if (state.body_dq.size() == static_cast<size_t>(H2_NUM_MOTOR)) {
            // Create temporary array for MuJoCo-ordered values
            std::array<double, H2_NUM_MOTOR> body_dq_mujoco;
            for (size_t i = 0; i < static_cast<size_t>(H2_NUM_MOTOR); ++i) {
        ...
        if (state.last_action.size() == static_cast<size_t>(H2_NUM_MOTOR)) {
            // Create temporary array for MuJoCo-ordered values
            std::array<double, H2_NUM_MOTOR> last_action_mujoco;
            for (size_t i = 0; i < static_cast<size_t>(H2_NUM_MOTOR); ++i) {
                // Convert to MuJoCo order (actions are in IsaacLab order in logger)
                last_action_mujoco[i] = state.last_action[isaaclab_to_mujoco[i]] * H2_action_scale[i] + default_angles[i];
            }
            // Pack in MuJoCo order
