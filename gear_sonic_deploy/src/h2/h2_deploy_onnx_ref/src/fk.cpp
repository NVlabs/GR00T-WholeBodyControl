...
RobotFK::RobotFK(const std::string &xmlfile)
{
    ...
    std::ofstream body_map_file("cpp_fk_body_map.txt");
    for(size_t body_id = 0; body_id < node_names_.size(); ++body_id)
    {
        body_map_file << "[C++ FK Body Map] " << body_id << ": " << node_names_[body_id] << '\n';
    }
}
