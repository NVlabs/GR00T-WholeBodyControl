/**
 * @file H2_deploy_onnx_ref.cpp
 * @brief Main application: deploys an RL locomotion policy on the Unitree H2 robot.
 *
 * This single-file application contains:
 *
 *   - **H2Deploy** class – the top-level controller that wires together all
 ...
 *   -----------------|----------|---------------------|-------------------------------
 *   Input            | 100 Hz   | H2Deploy::Input     | Poll input interface, handle commands.
 *   Control          | 50 Hz    | H2Deploy::Control   | Gather obs, run policy, compute motor targets.
 *   Planner          | 10 Hz    | H2Deploy::Planner   | Re-plan locomotion trajectory.
 *   Command Writer   | 500 Hz   | H2Deploy::LowCommandWriter | Publish motor commands via DDS.
 *
...
/**
 * @class H2Deploy
 * @brief Top-level controller that orchestrates the entire H2 deployment pipeline.
 *
...
class H2Deploy {
  private:
    ...
    std::array<double, H2_NUM_MOTOR> idle_readapt_original_targets_{};
    ...
    // Debug mode: after pressing start (']'), hold a fixed standing pose.
    // When enabled, we do NOT initialize or run policy/encoder/planner.
    bool debug_stand_pose_mode_ = false;
    std::array<float, H2_NUM_MOTOR> filtered_q_target_ = {};
    bool filtered_q_target_initialized_ = false;

     // Optional debug: use a more crouched default pose to reduce initial posture mismatch.
     // Enable with env var: GEAR_DEBUG_CROUCH_DEFAULT=1
     // IMPORTANT: If enabled, use this consistently anywhere `default_angles` is used.
     const std::array<double, H2_NUM_MOTOR>& DefaultAngles() const {
       static int crouch_mode = -1;
       if (crouch_mode < 0) {
         const char* v = std::getenv("GEAR_DEBUG_CROUCH_DEFAULT");
         crouch_mode = (v && std::string(v) == "1") ? 1 : 0;
       }

       // Tuning values (motor slot order): hip_pitch=-0.45, knee=0.90, ankle_pitch=-0.45
       // Arms/head/waist stay at existing neutral.
       static const std::array<double, H2_NUM_MOTOR> crouch_default_angles = {
         -0.45, 0.0, 0.0, 0.90, -0.45, 0.0,
         -0.45, 0.0, 0.0, 0.90, -0.45, 0.0,
         0.0, 0.0, 0.02,
         0.0, 0.0,
         0.15, 0.15, 0.0, 0.6, 0.0, 0.0, 0.0,
         0.15, -0.15, 0.0, 0.6, 0.0, 0.0, 0.0
       };

       return (crouch_mode == 1) ? crouch_default_angles : default_angles;
     }

     void PrintOscillationJointDebug(
       const MotorCommand& command,
       const std::array<double, H2_NUM_MOTOR>& policy_action_motor_order,
       const std::array<double, H2_NUM_MOTOR>& q_sent_formula
     ) const {
       if (logging_counter_ % 50 != 0) { return; }

       const auto low_state = used_low_state_data_.data;
       if (!low_state) { return; }

       constexpr std::array<int, 4> tracked_motor_indices = {16, 22, 21, 4};
        std::cout << "\n[OscillationJointDebug] action/default/scale/q_sent/current/gains (motor-slot order)" << std::endl;
        std::cout << std::left
                  << std::setw(26) << "joint"
                  << std::right
                  << std::setw(11) << "action"
                  ...
                  << std::setw(11) << "scale"
                  << std::setw(13) << "q_sent"
                  << std::setw(13) << "q_current"
                  << std::setw(10) << "kp"
                  << std::setw(10) << "kd"
                  << std::endl;

       for (int motor_idx : tracked_motor_indices) {
         const double q_current = low_state->motor_state()[motor_idx].q();
         std::cout << std::left << std::setw(26) << H2_MOTOR_JOINT_NAMES[motor_idx]
                   << std::right << std::fixed << std::setprecision(5)
                   << std::setw(11) << policy_action_motor_order[motor_idx]
                   << std::setw(11) << DefaultAngles()[motor_idx]
                   << std::setw(11) << H2_action_scale[motor_idx]
                   << std::setw(13) << q_sent_formula[motor_idx]
                   << std::setw(13) << q_current
                   << std::setw(10) << command.kp[motor_idx]
                   << std::setw(10) << command.kd[motor_idx]
                   << std::endl;
       }
     }

    // Per-motor high temperature hysteresis (enter at >= 90, exit at < 85)
    std::array<bool, H2_NUM_MOTOR> motor_high_temp_ = {};
    ...
    std::array<double, H2_NUM_MOTOR> last_action;
     ...
     *
     * Used by H2_dyn decoder in the refheading setting.
    ...
    /// Gather joint positions from N future frames. If joint_indexes is empty, gathers all joints.
    ...
    bool GatherMotionJointPositionsMultiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5, std::vector<int> joint_indexes = {}) {
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        if (joint_indexes.empty()) {
          size_t frame_offset = offset + frame_idx * H2_NUM_MOTOR;  // joints per frame
          for (int policy_idx = 0; policy_idx < H2_NUM_MOTOR; ++policy_idx) {
            const int motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
            target_buffer[frame_offset + policy_idx] = motion_joint_pos[motor_idx];
          }
          ...
          if (has_upper_body_data_) {
            std::array<double, H2_NUM_MOTOR> current_motion_joint_pos;
            for (int policy_idx = 0; policy_idx < H2_NUM_MOTOR; policy_idx++) {
              const int motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
              current_motion_joint_pos[policy_idx] = motion_joint_pos[motor_idx];
            }
            for (size_t i = 0; i < upper_body_external_17_isaaclab_order_in_isaaclab_index.size(); i++) {
              current_motion_joint_pos[upper_body_external_17_isaaclab_order_in_isaaclab_index[i]] = upper_body_joint_positions_buffer_[i];
            }
            std::copy(
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        if (joint_indexes.empty()) {
          ...
          if (operator_state.play) {
            for (int policy_idx = 0; policy_idx < H2_NUM_MOTOR; ++policy_idx) {
              const int motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
              target_buffer[frame_offset + policy_idx] = motion_joint_vel[motor_idx];
            }
            // If upper body control is enabled, use upper body joint velocities in buffer to replace the motion_joint_vel
            if (has_upper_body_data_) {
              std::array<double, H2_NUM_MOTOR> current_motion_joint_vel;
              for (int policy_idx = 0; policy_idx < H2_NUM_MOTOR; policy_idx++) {
                const int motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
                current_motion_joint_vel[policy_idx] = motion_joint_vel[motor_idx];
              }
              for (size_t i = 0; i < upper_body_external_17_isaaclab_order_in_isaaclab_index.size(); i++) {
                current_motion_joint_vel[upper_body_external_17_isaaclab_order_in_isaaclab_index[i]] = upper_body_joint_velocities_buffer_[i];
              }
              std::copy(
    ...
    bool GatherEncoderIndex(std::vector<double>& target_buffer, size_t offset) {
      target_buffer[offset] = static_cast<float>(current_motion_->GetEncodeMode());
      return true;
    }
    ...
    std::vector<ObservationRegistry> GetObservationRegistry() {
      ...
      return {{"token_state", token_dim, [this](std::vector<double>& buf, size_t offset) { return GatherTokenState(buf, offset); }},
              {"encoder_index", 1, [this](std::vector<double>& buf, size_t offset) { return GatherEncoderIndex(buf, offset); }},
              ...
              {"motion_joint_positions", H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointPositionsMultiFrame(buf, offset, 1, 1); }},
              {"motion_joint_velocities", H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointVelocitiesMultiFrame(buf, offset, 1, 1); }},
              ...
              {"motion_joint_positions_10frame_step5", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointPositionsMultiFrame(buf, offset, 10, 5); }},
              {"motion_joint_velocities_10frame_step5", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointVelocitiesMultiFrame(buf, offset, 10, 5); }},
              {"motion_joint_positions_10frame_step1", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointPositionsMultiFrame(buf, offset, 10, 1); }},
              {"motion_joint_velocities_10frame_step1", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointVelocitiesMultiFrame(buf, offset, 10, 1); }},
              ...
              {"motion_joint_positions_5frame_step5", 5 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointPositionsMultiFrame(buf, offset, 5, 5); }},
              {"motion_joint_velocities_5frame_step5", 5 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherMotionJointVelocitiesMultiFrame(buf, offset, 5, 5); }},
              ...
              {"body_joint_positions", H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisBodyJointPositions(buf, offset, 1, 1); }},
              {"body_joint_velocities", H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisBodyJointVelocities(buf, offset, 1, 1); }},
              {"last_actions", H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisLastActions(buf, offset, 1, 1); }},
              ...
              {"his_body_joint_positions_10frame_step1", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisBodyJointPositions(buf, offset, 10, 1); }},
              {"his_body_joint_velocities_10frame_step1", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisBodyJointVelocities(buf, offset, 10, 1); }},
              {"his_last_actions_10frame_step1", 10 * H2_NUM_MOTOR, [this](std::vector<double>& buf, size_t offset) { return GatherHisLastActions(buf, offset, 10, 1); }},
    ...
    void PrintH2OrderValidationIfRequested() const {
      static bool printed = false;
      if (printed) { return; }

      const char* v = std::getenv("GEAR_DEBUG_H2_ORDER");
      if (!v || std::string(v) != "1") { return; }
      printed = true;

      std::cout << "[H2Order] motor-slot order is DDS/LowCmd/LowState/default/action_scale order" << std::endl;
      std::cout << "[H2Order] policy order is ONNX action and joint-observation order" << std::endl;
      for (int motor_idx = 0; motor_idx < H2_NUM_MOTOR; ++motor_idx) {
        const int policy_idx = H2_MOTOR_TO_POLICY[motor_idx];
        const int roundtrip_motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
        std::cout << "[H2Order] motor[" << motor_idx << "] "
                  << H2_MOTOR_JOINT_NAMES[motor_idx]
                  << " -> policy[" << policy_idx << "] "
                  << H2_POLICY_JOINT_NAMES[policy_idx]
                  << " -> motor[" << roundtrip_motor_idx << "]" << std::endl;
        if (roundtrip_motor_idx != motor_idx) {
          std::cerr << "✗ Error: H2 order mapping round-trip mismatch at motor_idx="
                    << motor_idx << std::endl;
        }
      }
      std::cout << "[H2Order] external upper-body 17DOF buffer maps to policy joints without head DOFs:" << std::endl;
      for (size_t i = 0; i < upper_body_external_17_isaaclab_order_in_isaaclab_index.size(); ++i) {
        const int policy_idx = upper_body_external_17_isaaclab_order_in_isaaclab_index[i];
        std::cout << "[H2Order] upper17[" << i << "] -> policy[" << policy_idx << "] "
                  << H2_POLICY_JOINT_NAMES[policy_idx] << std::endl;
      }
    }

    std::string ResolveObservationConfigPath(const std::string& requested_path) const {
      namespace fs = std::filesystem;

      auto exists_regular = [](const fs::path& p) {
        std::error_code ec;
      ...
      const char* env_path = std::getenv("GEAR_H2_OBS_CONFIG");
      if (env_path && std::string(env_path).size() > 0) {
        candidates.emplace_back(env_path);
      }

      candidates.emplace_back("gear_sonic_deploy/policy/h2/observation_config.yaml");
      candidates.emplace_back("policy/h2/observation_config.yaml");
      candidates.emplace_back("../policy/h2/observation_config.yaml");
      ...
      if (!ec && !exe_path.empty()) {
        ...
        candidates.emplace_back(exe_dir / "../../policy/h2/observation_config.yaml");
        candidates.emplace_back(exe_dir / "../../../policy/h2/observation_config.yaml");
        candidates.emplace_back(exe_dir / "../../../../policy/h2/observation_config.yaml");
      }
      ...
      std::ostringstream oss;
      oss << "H2 observation config not found. Refusing to use default config. Tried:";
      for (const auto& candidate : candidates) {
        oss << "\n  - " << candidate.string();
      }
      throw std::runtime_error(oss.str());
    ...
    void SmoothUpperBodyTargets(MotorCommand& command) {
      static int smoothing_disabled = -1;
      if (smoothing_disabled < 0) {
        const char* v = std::getenv("GEAR_H2_DISABLE_UPPER_BODY_LPF");
        smoothing_disabled = (v && std::string(v) == "1") ? 1 : 0;
      }
      if (smoothing_disabled == 1) { return; }

      static double alpha = -1.0;
      if (alpha < 0.0) {
        alpha = 0.15;
        const char* v = std::getenv("GEAR_H2_UPPER_BODY_LPF_ALPHA");
        if (v) {
          try {
            alpha = std::stod(v);
          } catch (...) {}
        ...
        std::cout << "[H2Gentle] Upper-body q_target LPF alpha=" << alpha << std::endl;
      }

      if (!filtered_q_target_initialized_) {
        filtered_q_target_ = command.q_target;
        filtered_q_target_initialized_ = true;
        return;
      }

      static constexpr std::array<int, 16> upper_body_motor_indices = {
        HeadPitch, HeadYaw,
      ...
      for (int idx : upper_body_motor_indices) {
        filtered_q_target_[idx] = static_cast<float>(
          (1.0 - alpha) * static_cast<double>(filtered_q_target_[idx]) +
          alpha * static_cast<double>(command.q_target[idx]));
        command.q_target[idx] = filtered_q_target_[idx];
      }
    }
  ...
  public:
    ...
    H2Deploy(
      ...
      : time_(0.0),
        ...
        enable_motion_recording_(enable_motion_recording),
         ...
         initial_max_close_ratio_(initial_max_close_ratio),
          debug_stand_pose_mode_(debug_stand_pose_mode),
          //env(ORT_LOGGING_LEVEL_WARNING, "H2Deploy"),
          model_path(model_file_path),
          planner_path(planner_file_path) {
      PrintH2OrderValidationIfRequested();
      ...
      planner_motion_->ReserveCapacity(1500, H2_NUM_MOTOR, 1, 1, 0, 0);
      ...
      lowstate_subscriber_->InitChannel(std::bind(&H2Deploy::LowStateHandler, this, std::placeholders::_1), 1);
      imutorso_subscriber_.reset(new ChannelSubscriber<IMUState_>(HG_IMU_TORSO));
      imutorso_subscriber_->InitChannel(std::bind(&H2Deploy::imuTorsoHandler, this, std::placeholders::_1), 1);
      // Load motion data
      if (motion_reader_.ReadFromCSV(motion_data_path)) {
        if (!motion_reader_.motions.empty()) {
          ...
          if (const char* force_motion = std::getenv("GEAR_FORCE_MOTION_NAME")) {
            for (size_t i = 0; i < motion_reader_.motions.size(); ++i) {
              if (motion_reader_.motions[i] && motion_reader_.motions[i]->name == force_motion) {
                motion_reader_.current_motion_index_ = i;
                break;
              }
            }
          } else if (const char* force_index = std::getenv("GEAR_FORCE_MOTION_INDEX")) {
            const long requested_index = std::strtol(force_index, nullptr, 10);
            if (requested_index >= 0 && static_cast<size_t>(requested_index) < motion_reader_.motions.size()) {
              motion_reader_.current_motion_index_ = static_cast<size_t>(requested_index);
            }
          }
      ...
      if (debug_stand_pose_mode_) {
        std::cout << "[DebugStandPose] Enabled: will hold default standing pose after start ('])." << std::endl;
        std::cout << "[DebugStandPose] Skipping policy/encoder/planner initialization." << std::endl;
        // Ensure token-related safety logic is disabled in this mode.
      ...
      const std::string resolved_obs_config_path = ResolveObservationConfigPath(obs_config_path);
      std::cout << "Using H2 observation config file: " << resolved_obs_config_path << std::endl;
      full_obs_config = ObservationConfigParser::ParseFullConfig(resolved_obs_config_path);
      ...
      if (!debug_stand_pose_mode_ && !encoder_file_path.empty() && encoder_config_.dimension > 0) {
      ...
      // Initialize planner
      if (!debug_stand_pose_mode_ && !planner_path.empty()) {
      ...
      robot_config["obs_config_path"] = resolved_obs_config_path;
      ...
      try {
        ...
        state_logger_ = std::make_unique<StateLogger>(resolved_logs_dir, 10000, H2_NUM_MOTOR, H2_NUM_MOTOR, 
      ...
      if (input_type == "gamepad") {
        ...
        input_interface_ = std::make_unique<ROS2InputHandler>(true, "H2_deploy_ros2_handler");
...
#if HAS_ROS2
      if (create_ros2) {
        auto ros2_handler = std::make_unique<ROS2OutputHandler>(*state_logger_, "H2_output_handler");
      ...
      // create threads
      input_thread_ptr_ = CreateRecurrentThreadEx("Input", UT_CPU_ID_NONE, input_dt_ * 1e6, &H2Deploy::Input, this);
      command_writer_ptr_ = CreateRecurrentThreadEx("command_writer", UT_CPU_ID_NONE, publish_dt_ * 1e6,
                                                    &H2Deploy::LowCommandWriter, this);
      control_thread_ptr_ =
          CreateRecurrentThreadEx("control", UT_CPU_ID_NONE, control_dt_ * 1e6, &H2Deploy::Control, this);
      
      if (planner_) {
        planner_thread_ptr_ =
          CreateRecurrentThreadEx("planner", UT_CPU_ID_NONE, planner_dt_ * 1e6, &H2Deploy::Planner, this);
      }
          
      // In local MuJoCo sim2sim we run deploy + simulator on the same machine.
      // Aggressive realtime scheduling (SCHED_FIFO + CPU pinning) can starve the
      // Python simulator process, causing LowState to stop and triggering safety.
      // Only enable realtime priority when explicitly requested.
      const char* rt_env = std::getenv("GEAR_ENABLE_RT");
      const bool enable_rt = (rt_env && std::string(rt_env) == "1");
      if (enable_rt) {
        // SetThreadPriority();
      }
    }
    ...
    void LowStateHandler(const void* message) {
      ...
      {
        std::array<uint32_t, H2_NUM_MOTOR> motorstates;
    ...
    void LowCommandWriter() {
      ...
      if (mc) {
        // Optional debug: verify we are publishing non-trivial commands.
        // Enable with env var: GEAR_DEBUG_DDS_LOW_CMD=1
        static int debug_dds_lowcmd = -1;
        if (debug_dds_lowcmd < 0) {
          const char* v = std::getenv("GEAR_DEBUG_DDS_LOW_CMD");
          debug_dds_lowcmd = (v && std::string(v) == "1") ? 1 : 0;
        }
        ...
        if (debug_dds_lowcmd == 1) {
          static uint64_t dbg_counter = 0;
          if ((dbg_counter++ % 500) == 0) {  // ~1Hz at 500Hz writer
            std::cout << "[DebugDDSLowCmd] q_target[0:3]="
                      << mc->q_target.at(0) << "," << mc->q_target.at(1) << "," << mc->q_target.at(2)
                      << " kp[0:3]="
                      << mc->kp.at(0) << "," << mc->kp.at(1) << "," << mc->kp.at(2)
                      << " mode_machine=" << static_cast<int>(mode_machine_)
                      << std::endl;
          }
        }
      }
    ...
    bool InitControl() {
      ...
      MotorCommand motor_command_tmp;
      const auto& defaults = DefaultAngles();
      for (int i = 0; i < H2_NUM_MOTOR; ++i) {
        motor_command_tmp.tau_ff.at(i) = 0.0;
        motor_command_tmp.q_target.at(i) = static_cast<float>(defaults[i]);
        motor_command_tmp.dq_target.at(i) = 0.0;
      ...
      if (time_ < duration_) {
        for (int i = 0; i < H2_NUM_MOTOR; i++) {
          ...
          motor_command_tmp.q_target.at(i) =
              static_cast<float>(current_pos * (1.0 - ratio) + defaults[i] * ratio);
        }
      ...
      SmoothUpperBodyTargets(motor_command_tmp);
      motor_command_buffer_.SetData(motor_command_tmp);
      return true;
    }
     ...
     * @brief Read the latest robot state (IMU + joints + hands) and log it.
      ...
      * from motor-slot order to policy/IsaacLab order (subtracting motor-slot defaults), and
    ...
    bool GatherRobotStateToLogger() {
      ...
      // robot state data
      std::array<double, H2_NUM_MOTOR> body_q = {0.0};
      std::array<double, H2_NUM_MOTOR> body_dq = {0.0};

      auto unitree_joint_state = ls->motor_state();
      std::array<double, H2_NUM_MOTOR * 2> motor_temperature = {0.0};
      std::array<double, H2_NUM_MOTOR> motor_error = {0.0};
      std::array<double, H2_NUM_MOTOR> motor_torque = {0.0};
      const auto& defaults = DefaultAngles();
      for (int policy_idx = 0; policy_idx < H2_NUM_MOTOR; policy_idx++) {
        const int motor_idx = H2_POLICY_TO_MOTOR[policy_idx];
        body_q[policy_idx] =
            unitree_joint_state[motor_idx].q() - defaults[motor_idx];
        body_dq[policy_idx] = unitree_joint_state[motor_idx].dq();
        if (std::abs(body_dq[policy_idx]) > 35 && !disable_crc_check_) {
          std::cout << "✗ Error: body_dq[" << policy_idx << "] = " << body_dq[policy_idx] << " > 35."
                    << std::endl;
          return false;
        }
        // Extract motor temperature (2 values per motor: winding, driver) in hardware order
        motor_temperature[motor_idx * 2]     = static_cast<double>(unitree_joint_state[motor_idx].temperature()[0]);
        motor_temperature[motor_idx * 2 + 1] = static_cast<double>(unitree_joint_state[motor_idx].temperature()[1]);
        // Extract motor error code in hardware order
        motor_error[motor_idx] = static_cast<double>(unitree_joint_state[motor_idx].motorstate());
        // Extract estimated torque in hardware order
        motor_torque[motor_idx] = static_cast<double>(unitree_joint_state[motor_idx].tau_est());

        // High temperature hysteresis check (enter >= 90, exit < 85)
        int16_t max_temp = std::max(unitree_joint_state[motor_idx].temperature()[0],
                                    unitree_joint_state[motor_idx].temperature()[1]);
        if (motor_high_temp_[motor_idx]) {
          if (max_temp < HIGH_TEMP_EXIT) { motor_high_temp_[motor_idx] = false; }
        } else {
          if (max_temp >= HIGH_TEMP_ENTER) { motor_high_temp_[motor_idx] = true; }
        }
      }
     ...
     * (hardware order) using `H2_action_scale` and `default_angles`.
     */
    bool CreatePolicyCommand() {
      // Deploy-side HOLD_DEFAULT: bypass policy actions but still publish LowCmd
      // through the normal deploy pipeline.
      // Enable with env var: GEAR_DEBUG_HOLD_DEFAULT=1
      static int hold_default = -1;
      if (hold_default < 0) {
        const char* v = std::getenv("GEAR_DEBUG_HOLD_DEFAULT");
        hold_default = (v && std::string(v) == "1") ? 1 : 0;
      }
      if (hold_default == 1) {
        static bool printed = false;
        if (!printed) {
          std::cout << "[debug] GEAR_DEBUG_HOLD_DEFAULT=1: policy action forced to 0, holding deploy default_angles" << std::endl;
          printed = true;
        }

        MotorCommand motor_command_tmp;
        const auto& defaults = DefaultAngles();
        std::array<double, H2_NUM_MOTOR> policy_action_motor_order = {0.0};
        std::array<double, H2_NUM_MOTOR> q_target_formula = {0.0};
        for (int i = 0; i < H2_NUM_MOTOR; i++) {
          last_action[i] = 0.0;
          q_target_formula[i] = defaults[i];
          motor_command_tmp.q_target.at(i) = static_cast<float>(defaults[i]);
          motor_command_tmp.tau_ff.at(i) = 0.0;
          motor_command_tmp.kp.at(i) = kps[i];
          motor_command_tmp.kd.at(i) = kds[i];
        ...
        SmoothUpperBodyTargets(motor_command_tmp);
        PrintOscillationJointDebug(motor_command_tmp, policy_action_motor_order, q_target_formula);
        motor_command_buffer_.SetData(motor_command_tmp);
        return true;
      }
      ...
      // Optional debug: print action/q_target stats to verify commands are non-trivial.
      // Enable with env var: GEAR_DEBUG_POLICY_CMD=1
      static int debug_policy_cmd = -1;
      if (debug_policy_cmd < 0) {
        const char* v = std::getenv("GEAR_DEBUG_POLICY_CMD");
        debug_policy_cmd = (v && std::string(v) == "1") ? 1 : 0;
      }
      const bool do_debug = (debug_policy_cmd == 1) && (logging_counter_ % 50 == 0);

      MotorCommand motor_command_tmp;
      const auto& defaults = DefaultAngles();
      std::array<double, H2_NUM_MOTOR> policy_action_motor_order = {0.0};
      std::array<double, H2_NUM_MOTOR> q_target_formula = {0.0};
      double a_min = 1e30, a_max = -1e30, a_abs_mean = 0.0;
      for (int i = 0; i < H2_NUM_MOTOR; i++) {
        const double a_il = static_cast<double>(floatarr[i]);
        a_min = std::min(a_min, a_il);
        a_max = std::max(a_max, a_il);
        a_abs_mean += std::abs(a_il);

        // Safety fuse for sim2sim debugging:
        // If observations go unstable, policy outputs can blow up and explode MuJoCo.
        // Clamp only when debug printing is enabled so default behavior is unchanged.
        const int policy_idx = H2_MOTOR_TO_POLICY[i];
        double raw_action = static_cast<double>(floatarr[policy_idx]);
        if (debug_policy_cmd == 1) {
          raw_action = std::clamp(raw_action, -0.5, 0.5);
        }
        const double action_value = raw_action * H2_action_scale[i];
        policy_action_motor_order[i] = raw_action;
        q_target_formula[i] = defaults[i] + action_value;
        last_action[policy_idx] = raw_action;
        motor_command_tmp.q_target.at(i) = static_cast<float>(defaults[i] + action_value);
        motor_command_tmp.tau_ff.at(i) = 0.0;
        motor_command_tmp.kp.at(i) = kps[i];
        motor_command_tmp.kd.at(i) = kds[i];
        motor_command_tmp.dq_target.at(i) = 0.0;
      ...
      if (do_debug) {
        a_abs_mean /= static_cast<double>(H2_NUM_MOTOR);
        std::cout << "[DebugPolicyCmd] action(isaaclab) min=" << a_min
                  << " max=" << a_max << " mean_abs=" << a_abs_mean
                  << " | q_target[0:3]="
                  << motor_command_tmp.q_target.at(0) << ","
                  << motor_command_tmp.q_target.at(1) << ","
                  << motor_command_tmp.q_target.at(2)
                  << std::endl;
      }
      motor_command_buffer_.SetData(motor_command_tmp);
      return true;
        ...
        if (ls) {
          if (!planner_->planner_state_.initialized) {
            try {
              ...
              std::array<double, H2_NUM_MOTOR> joint_positions;
              for (int i = 0; i < H2_NUM_MOTOR; i++) {
      ...
      switch (program_state_) {
        ...
        case ProgramState::WAIT_FOR_CONTROL:
          ...
          if (operator_state.start) {
            ...
            if (!debug_stand_pose_mode_ && initial_encoder_mode_ == -1 && !first_token_received_) {
            ...
            if (debug_stand_pose_mode_) {
              std::cout << "[debug] PURE_PD_STAND_MODE active (policy/encoder/planner bypassed)" << std::endl;
            }
        ...
        case ProgramState::CONTROL: {
          ...
          // Debug: hold default standing pose after start, bypassing policy/encoder/planner.
          if (debug_stand_pose_mode_) {
            MotorCommand motor_command_tmp;
            std::array<double, H2_NUM_MOTOR> policy_action_motor_order = {0.0};
            std::array<double, H2_NUM_MOTOR> q_target_formula = {0.0};
            ...
            // Keep output publishing alive (optional) but skip all other pipeline stages.
            for (auto& output_interface : output_interfaces_) {
              if (output_interface) {
                output_interface->publish(
                  ...
                  heading_state_buffer_, current_motion_, current_frame_
                );
              }
            }
            break;
          ...
          if (report_temperature_) {
            ...
            if (ls) {
              ...
              std::array<std::pair<int16_t, int>, H2_NUM_MOTOR> max_temps;
              for (int i = 0; i < H2_NUM_MOTOR; ++i) {
              ...
              for (int k = 0; k < 3 && k < H2_NUM_MOTOR; ++k) {
...
/**
 * @brief Entry point: parse CLI arguments and run the H2 deployment application.
 *
...
int main(int argc, char const* argv[]) {
  ...
  if (argc < 4) {
    ...
    std::cout << "  --debug-stand-pose: after pressing ']': hold default standing pose; skip policy/encoder/planner" << std::endl;
    ...
    std::cout << "  --zmq-out-topic <topic>: ZMQ topic/prefix for output (default: H2_debug)" << std::endl;
  ...
  for (int i = 4; i < argc; i++) {
    ...
    } else if (std::string(argv[i]) == "--debug-stand-pose") {
      debugStandPose = true;
      std::cout << "[INFO] Debug stand pose enabled: policy/encoder/planner will be skipped" << std::endl;
  ...
  std::cout << "[DEBUG] Creating H2Deploy object..." << std::endl;
  H2Deploy custom(
    networkInterface,
    modelFile,
    motionDataPath,
  ...
  std::cout << "[DEBUG] H2Deploy object created successfully!" << std::endl;
  ...
  std::cout << "[DEBUG] Stopping H2Deploy..." << std::endl;
  custom.Stop();
