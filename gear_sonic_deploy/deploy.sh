...
# Keep basic system tools reachable even if the parent shell has a damaged PATH.
...
# Just is convenient, but the snap-packaged binary can fail under some shells.
# Keep direct CMake/executable fallbacks so deployment is not blocked by snap.
just_available() {
    command -v just &> /dev/null && just --version &> /dev/null
}

build_project() {
    if just_available; then
        just build
        return
    fi

    echo -e "${YELLOW}⚠️  just is not usable; building directly with CMake${NC}"
    mkdir -p build
...
if just_available; then
    ...
    echo -e "${YELLOW}⚠️  just is unavailable or not usable; CMake fallback will be used${NC}"
fi
...
if [[ "$confirm" =~ ^[Yy]$ ]] || [[ -z "$confirm" ]]; then
    ...
    if [[ -n "$EXTRA_ARGS" ]]; then
        run_deploy_executable "$TARGET" "$CHECKPOINT_DECODER" "$MOTION_DATA" \
