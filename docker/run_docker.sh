...
# Docker run script for gr00t with branch-based container isolation
# 
# Usage:
...
#   - Container names include branch identifier (e.g., gr00t-deploy-user-main)
...
# When true, the image will be tagged and pushed to docker.io/nvgear/GR00T-WholeBodyControl:latest (lowercased in practice)
...
BRANCH_MODE=false
EXTRA_ARGS=()
PROJECT_NAME="GR00T-WholeBodyControl"
...
if [ "$ROOT" = true ]; then
    ...
    CACHE_FROM=gr00t-deploy-cache-root
else
    USERNAME=$(whoami)
    USERID=$(id -u)
    DOCKER_HOME_DIR=/home/${USERNAME}
    CACHE_FROM=gr00t-deploy-cache
fi
...
DOCKER_RUN_ARGS="--hostname $HOSTNAME \
    ...
    -e USERNAME=$USERNAME \
    -e GR00T_DIR="$DOCKER_HOME_DIR/Projects/$WORKTREE_NAME" \
...
if [ "$DEPLOY" = true ]; then
    # Deploy mode - use gr00t-deploy-${USERNAME} container
   
    # Always clean up old processes and create a new container
    # Kill all gr00t processes across containers to prevent message passing conflicts
    "$SCRIPT_DIR/kill_gr00t_processors.sh"
    ...
    # Bash mode - use gr00t-bash-${USERNAME} container
    if sudo docker ps -a --format '{{.Names}}' | grep -q "^$BASH_CONTAINER$"; then
        ...
        echo "Creating new bash container with auto-install gr00t..."
