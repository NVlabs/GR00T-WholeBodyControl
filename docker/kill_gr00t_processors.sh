...
# kill_gr00t_processors.sh
# Kill gr00t processes in current container to prevent message passing conflicts
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "Kill gr00t processes to prevent message passing conflicts"
            exit 0 ;;
...
main() {
    ...
    if [[ "$QUIET" != true ]]; then
        echo -e "${BLUE}=== GR00T Process Killer ===${NC}"
        ...
        if [[ "$FORCE" != true && "$DRY_RUN" != true ]]; then
            echo
            echo -e "${RED}WARNING: This will terminate the above gr00t processes!${NC}"
    ...
    [[ "$QUIET" != true ]] && {
        if [[ "$DRY_RUN" == true ]]; then
            ...
            echo -e "${GREEN}All gr00t processes terminated${NC}"
        fi
    }
}
