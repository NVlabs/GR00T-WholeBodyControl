...
def main():
    ...
    # ------------------------------------------------------------------
    # DEBUG: force foot geom contact margin to zero
    # ------------------------------------------------------------------
    for gname in ["left_foot_collision", "right_foot_collision"]:
        ...
        if gid >= 0:
            print(
                "[FOOT_MARGIN_BEFORE]",
                gname,
                "margin=", mj_model.geom_margin[gid],
                "gap=", mj_model.geom_gap[gid],
                flush=True,
            )

            mj_model.geom_margin[gid] = 0.0
            mj_model.geom_gap[gid] = 0.0
    ...
    def apply_rsi(frame_idx: int):
        ...
        if processed_motion_ref is not None:
            ...
            joint_pos_mj = joint_pos_il.numpy()
            joint_vel_mj = joint_vel_il.numpy()
    ...
    try:
        while current_frame < max_frames and (viewer is None or viewer.is_running()):
            ...
            debug_dump_steps = {0, 1, 2, 3, 4, 5, 10}
            contact_pairs = []
            contact_forces = []
            contact_dists = []
            contact_pos = []
            ...
            for ci in range(mj_data.ncon):
                ...
                mujoco.mj_contactForce(mj_model, mj_data, ci, force6)

                contact_pairs.append(f"{name1}<->{name2}")
                contact_forces.append(force6.copy())

                # 추가
                contact_dists.append(float(c.dist))
            ...
            if _l_cg is not None:
                left_foot_z = compute_foot_obb_lowest_z(_l_cg)[0]
            if _r_cg is not None:
                right_foot_z = compute_foot_obb_lowest_z(_r_cg)[0]
            if total_steps in debug_dump_steps:
                torch.save({
                    "step": total_steps,
                    "frame": current_frame,
                    "action_il": action_il.detach().cpu(),
                    "target_il": target_il.detach().cpu(),
                    "target_mj": target_mj.detach().cpu(),
                    "ctrl_mj": torch.as_tensor(mj_data.ctrl.copy()).float(),
                    "qpos_mj": current_mj.detach().cpu(),
                    "qvel_mj": vel_mj.detach().cpu(),
                    "root_pos_mj": torch.as_tensor(mj_data.qpos[:3].copy()).float(),
                    "root_quat_mj": torch.as_tensor(mj_data.qpos[3:7].copy()).float(),
                    "root_lin_vel_mj": torch.as_tensor(mj_data.qvel[:3].copy()).float(),
                    "root_ang_vel_mj": torch.as_tensor(mj_data.qvel[3:6].copy()).float(),
                    "actor_obs": obs_dict["actor_obs"].detach().cpu(),
                    "tokenizer": obs_dict["tokenizer"].detach().cpu(),
                    "contact_count": int(mj_data.ncon),
                    "contact_forces": torch.as_tensor(np.asarray(contact_forces)).float(),
                    "left_foot_lowest_z": left_foot_z,
                    "right_foot_lowest_z": right_foot_z,
                    "contact_pairs": contact_pairs,
                    "contact_forces": torch.as_tensor(np.asarray(contact_forces)).float(),
                    "contact_dists": torch.as_tensor(np.asarray(contact_dists)).float(),
                    "contact_pos": torch.as_tensor(np.asarray(contact_pos)).float(),
                }, f"dump_step0/h2_policy_step{total_steps:03d}_mujoco.pt")
                print(f"[DUMP] saved step {total_steps}", flush=True)
            # --- Step physics ---
