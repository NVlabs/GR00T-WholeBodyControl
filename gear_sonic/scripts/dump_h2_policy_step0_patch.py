...
def _install_forward_dump_patch():
    ...
    def patched_forward(self, input_data, compute_aux_loss=False, return_dict=False, **kwargs):
        ...
        # 원하는 step 아니면 저장 안 하고 그대로 반환
        if call_idx not in debug_dump_steps:
            return out

        print(f"[dump_isaaclab] UniversalTokenModule.forward call {call_idx} — capturing.",
            flush=True)
        ...
        save_path = _DUMP_PATH.replace(
            ".pt",
            f"_step{call_idx:03d}.pt"
        )

        torch.save(dump, save_path)

        print(f"[dump_isaaclab] Wrote dump to {save_path}", flush=True)
