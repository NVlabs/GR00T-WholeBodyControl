...
import gr00t
from gr00t.control.main.config_template import ArgsConfig as ArgsConfigTemplate
from gr00t.control.policy.wbc_policy_factory import WBC_VERSIONS
from gr00t.control.utils.network_utils import resolve_interface
...
class BaseConfig(ArgsConfigTemplate):
    ...
    """Path to WBC model file (relative to gr00t/sim2mujoco/resources/robots/g1)"""
    """gear_wbc model path: policy/stand.onnx,policy/walk.onnx"""
    ...
    def load_wbc_yaml(self) -> dict:
        ...
        # Get the base path to gr00t and convert to Path object
        gr00t_path = Path(os.path.dirname(gr00t.__file__))

        if self.wbc_version == "gear_wbc":
            config_path = str(gr00t_path / "control/main/teleop/configs/g1_29dof_gear_wbc.yaml")
        else:
            raise ValueError(
